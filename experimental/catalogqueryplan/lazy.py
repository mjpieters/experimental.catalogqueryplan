...
def patch_lazy():
    from Products.ZCatalog import Lazy as _lazy
    _lazy.Lazy = Lazy
    _lazy.LazyCat = LazyCat
    _lazy.LazyMap = LazyMap
    _lazy.LazyFilter = LazyFilter
    _lazy.LazyMop = LazyMop
    _lazy.LazyValues = LazyValues

    from Products.ATContentTypes.content import topic
    topic.LazyCat = LazyCat

    try:
        from collective.dancing.patch import topic as ptopic
    except ImportError:
        pass
    else:
        ptopic.LazyCat = LazyCat
